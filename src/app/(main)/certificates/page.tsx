...
import type { CertificateItem as CertificateItemType } from "@/lib/constants";
  ...
  return (
    <div className="space-y-12">
      <section className="text-center mb-16">
        ...
        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-primary">Certificates</h1>
      ...
      {certificateItems && certificateItems.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {certificateItems.map((item: CertificateItemType) => {
            const ItemIcon = getIcon(item.iconName) || Award;
            return (
              <Card key={item.id} className="flex flex-col bg-card/70 border-border/30 rounded-lg shadow-lg hover:shadow-accent/20 transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                {item.imageUrl && (
                  <div className="relative w-full h-48 sm:h-60"> {/* Adjusted height for better proportion */}
                    <Image
                      src={item.imageUrl}
                      alt={item.title}
                      fill
